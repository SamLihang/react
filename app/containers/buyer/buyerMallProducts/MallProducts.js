...
import {ScrollView, StyleSheet, TextInput, View, ListView, FlatList} from "react-native";
...
import {ActionLoadMallProducts, ActionMallProducts} from '../../../actions/ProductAction';
import {
        ...
        return (
            <View style={[styles.cartItem]} key={item.ProductName + index}>
                ...
                <View style={[styles.cartItemRight]}>
                    <BCText style={[styles.cartItemPrice, gs.fts_13, gs.c_fd0319]}>￥{item.Price}</BCText>
                    <Calculate
                        ref={(c) => {
                            this._Product = c;
                        }}
                        ...
                        OnChange={(type, quantity, shoppingCartId) => {
                            ...
                            this.props.OnCount(type, quantity, shoppingCartId, item.ProductGlobalId, item.SpecId, item.Price, item.SpecName, item.ProductName, item.DisplayUnitTypeId, item.UnitAmount, item.CompanyId)
                        }}/>
                </View>
            </View>
        )
...
class Dl extends Component {
    ...
    render() {
        ...
        let productName = substr(item.ProductName, 11);
...
class MallProducts extends PageComponent {
    _page = 1
    _Products = {}
            ...
            <View style={gs.bgc_fff}>
                <View style={[styles.company]}>
                    ...
                    <View style={styles.companyContent}>
                        <BCText style={[gs.fts_16, gs.c_3a3838,]}>{provider.CompanyName}</BCText>
    ...
    maksContent() {
        ...
        return (
            <ShoppingCartLists
                ...
                dispatch={dispatch}
                OnCount={(type, quantity, shoppingCartId, productId, SpecId, price, specName, productName, displayUnitTypeId, unitAmount, companyId) => {
                    ...
                    if (_dl) {
                        ...
                        productDl.props.OnPressCalculate({
                            ...
                            productName, displayUnitTypeId, unitAmount, companyId
                        });
                    }
                }}
                ClearAll={() => {
    ...
    content() {
        ...
        //有数据  加载 无数据跳过
        if(this.GlobalDatas.hasOwnProperty('Provider')) {

            let {ParentCategorys, ChildCategorys, Products, Provider} = this.GlobalDatas;


            return (
                <View style={[styles.main, gs.bgc_f2f1ef]}>
                    {this.renderCompany(Provider.BCompanyId, Provider.SCompanyId, Provider)}
    ...
    WillReceive(nextProps) {
        ...
        if (nextProps.ReduceLoadMallProducts.datas != null && nextProps.ReduceLoadMallProducts.datas != this.props.ReduceLoadMallProducts.datas) {
            ...
            let {Categorys, Product, Carts, Provider} = ReduceLoadMallProducts.datas;
        ...
        if (nextProps.ReduceMallProducts.datas != null && nextProps.ReduceMallProducts.datas != this.props.ReduceMallProducts.datas) {
            const {ReduceMallProducts} = nextProps;
            const products = ReduceMallProducts.datas.Product;
            const Carts = ReduceMallProducts.datas.Carts;
            this.Carts = Carts;
...
function mapStateToProps(store) {
    return {
        ...
        ReduceMallProducts: store.ReduceMallProducts,
...
const connectProviders = connect(mapStateToProps)(MallProducts);
