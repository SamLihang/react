/**
 * Created by Administrator on 2017/4/10.待接单
 */
...
import {ActionAcceptSalesOrder, ActionSalesOrderDetail} from "../../../actions/SellerSalesOrderAction";
...
class Detail1 extends PullViewComponent {
    //设置页面标题
    setTitle() {
        return "订单详情(待接单)"
    }
    ...
    Bottom() {
        return (
            this.renderBottom(this.Amount, this.DeliveryAmount)
        )
    }
                ...
                <View style={Styles.singleFooter}>
                    ...
                    <BCTouchable onPress={() => {
                        this.AcceptSalesOrder();
                                 ...
                                 style={[gs.bgc_BaseColor, Styles.pay]}>
                        <BCText style={[gs.c_fff, gs.fts_17, gs.bold]}>接单</BCText>
                    </BCTouchable>
                </View>
            </View>
        )
    ...
    AcceptSalesOrder() {
        ...
        if (salesOrderIds.length > 0) {
            ...
            //toastShort("你已接单成功");
            //dispatch(ActionSellerSalesOrder1({p: 1, t: 1}));
            //dispatch(ActionSellerSalesOrder3({p: 1, t: 3}));
        }
    ...
    WillReceive(nextProps) {
        ...
        if (nextProps.ReduceAcceptSalesOrder.datas != null && nextProps.ReduceAcceptSalesOrder.datas !== this.props.ReduceAcceptSalesOrder.datas) {
            ...
            if(this.params.pageForm&&this.params.pageForm=="Jpush"){
                toastShort("你已接单成功");
                this.push("SellerList", {initialPage: 0,pageForm:"Jpush"})
            }
            const {navigation} = this.props;
...
const connectDetail1 = connect(mapStateToProps)(Detail1);
connectDetail1.navigationOptions = NavigationOptions;
export default connectDetail1;