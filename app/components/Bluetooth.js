...
class BluetoothSerialExample extends Component {
    ...
    componentWillMount () {
        ...
        BluetoothSerial.on('bluetoothEnabled', () => toastShort('Bluetooth enabled'))
        BluetoothSerial.on('bluetoothDisabled', () => toastShort('Bluetooth disabled'))
        BluetoothSerial.on('error', (err) => console.log(`Error: ${err.message}`))
        BluetoothSerial.on('connectionLost', () => {
            if (this.state.device) {
                toastShort(`Connection to device ${this.state.device.name} has been lost`)
            }
    ...
    connect (device) {
        ...
        BluetoothSerial.connect(device.id)
            .then((res) => {
                toastShort(`Connected to device ${device.name}`)
